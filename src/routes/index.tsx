...
import { Filter, Brain, ShieldCheck, Gauge, Check, X, Zap, Link as LinkIcon, Target, MessageCircle, Star, TrendingUp, Users, Trophy, Flame, Eye, ArrowRight } from "lucide-react";
...
export const Route = createFileRoute("/")(({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Real-time whale alerts on Polymarket. Verified ROI wallets, AI analysis, auto-audited track record. Free channel + VIP $15/mo." },
        ...
        <div className="max-w-4xl mx-auto text-center stagger">
          ...
          <div className="mt-8 flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center items-center">
            ...
            <button onClick={onUpgradeClick}
              className="btn-gradient-cta inline-flex items-center justify-center gap-2 rounded-xl px-6 py-3.5 font-display text-sm sm:text-base font-semibold w-full sm:w-auto">
              Get VIP Access — $15/mo <span aria-hidden>↗</span>
            </button>
          </div>
...
function LiveActivityBar() {
  ...
  useEffect(() => {
    const base = Math.floor(Math.random() * 40) + 180;
    setCount(base);
    const t = setInterval(() => {
  ...
  const events = [
    "🐋 El Oráculo just entered Trump 2026 — $2,400",
    "⚡ ALTA CONVICCIÓN — La Sombra doubled down on Fed cut",
    "🏛️ CONSENSO — 3 whales on Will Russia invade Ukraine?",
    "🔄 CONTRARIAN — El Estratega going NO on 78% market",
    "🆕 NEW WHALE — First signal detected, $3,200 entry",
    "✅ VERIFIED WIN — El Arquitecto +$1,840 ROI confirmed",
  ];

  return (
    <div className="border-y border-neon/10 bg-neon/3 py-2.5 overflow-hidden">
      <div className="flex items-center gap-3 px-4">
        <span className="shrink-0 flex items-center gap-1.5 text-neon font-display text-xs font-bold uppercase tracking-widest">
        ...
        <div className="overflow-hidden flex-1">
          <div className="flex gap-8 animate-[marquee_25s_linear_infinite] whitespace-nowrap">
            {[...events, ...events].map((e, i) => (
              <span key={i} className="text-xs text-muted-foreground font-display shrink-0">{e}</span>
            ))}
          </div>
        </div>
        <span className="shrink-0 text-xs text-muted-foreground font-display">
          <span className="text-neon font-bold">{count.toLocaleString()}</span> whales tracked
        </span>
      </div>
    </div>
  );
...
function Features() {
  const newFeatures = [
    { icon: "🐋", title: "Whale Profile Cards", desc: "Every wallet gets an epic alias. Track El Oráculo's 8/10 win streak, La Sombra's contrarian bets — follow your favorites." },
    { icon: "🏛️", title: "Institutional Consensus", desc: "When 3+ different whales bet the same outcome within 1 hour → instant alert. Smart money converging = highest-probability signal." },
    { icon: "🔄", title: "Contrarian Alerts", desc: "A wallet with 40%+ ROI bets NO on a 75% YES market? That's insider-level alpha. We flag it immediately." },
    { icon: "🚪", title: "Exit Detection", desc: "When a whale starts selling their position, you know before the market moves. First to know, first to act." },
  ];
      ...
      <div className="container mx-auto px-5 sm:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-5 max-w-5xl mx-auto">
          <div className="group relative rounded-3xl border border-border bg-card/30 p-6 sm:p-8 hover:bg-card/60 hover:border-electric/40 transition-all duration-500 overflow-hidden md:col-span-2 md:row-span-1 flex flex-col justify-center cursor-default">
            ...
            <div className="inline-flex h-12 w-12 items-center justify-center rounded-xl bg-electric/10 border border-electric/30 text-electric mb-5">
              <Brain className="h-6 w-6" />
            </div>
            <h3 className="font-display text-xl sm:text-2xl font-bold mb-3">AI Analysis Powered by Claude</h3>
          ...
          <div className="group relative rounded-3xl border border-border bg-card/30 p-6 sm:p-8 hover:bg-card/60 hover:border-amber-score/40 transition-all duration-500 overflow-hidden md:col-span-1 flex flex-col justify-between cursor-default">
            <div>
              <div className="inline-flex h-12 w-12 items-center justify-center rounded-xl bg-amber-score/10 border border-amber-score/30 text-amber-score mb-5">
                <Gauge className="h-6 w-6" />
              </div>
              <h3 className="font-display text-xl font-bold mb-3">Confidence Score 0–100</h3>
          ...
          <div className="group relative rounded-3xl border border-border bg-card/30 p-6 sm:p-8 hover:bg-card/60 hover:border-neon/40 transition-all duration-500 overflow-hidden md:col-span-1 cursor-default">
            <div className="inline-flex h-12 w-12 items-center justify-center rounded-xl bg-neon/10 border border-neon/30 text-neon mb-5">
              <Filter className="h-6 w-6" />
            </div>
            <h3 className="font-display text-xl font-bold mb-3">ROI Filtering</h3>
            <p className="text-sm text-muted-foreground leading-relaxed">
          ...
          <div className="group relative rounded-3xl border border-neon/30 bg-[color-mix(in_oklab,var(--card)_85%,var(--neon)_5%)] p-6 sm:p-8 hover:bg-[color-mix(in_oklab,var(--card)_75%,var(--neon)_8%)] transition-all duration-500 overflow-hidden md:col-span-2 flex flex-col justify-center cursor-default">
            <span className="absolute right-5 top-5 inline-flex items-center gap-1.5 rounded-full border border-amber-score/40 bg-amber-score/10 text-amber-score font-display text-[10px] uppercase tracking-widest px-3 py-1.5">★ Unique to us</span>
            ...
            <h3 className="font-display text-xl sm:text-2xl font-bold mb-3 pr-32 sm:pr-0">Auto-Resolution Audit</h3>
        ...
        {/* NEW FEATURES GRID */}
        <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 max-w-5xl mx-auto">
          {newFeatures.map((f, i) => (
            <div key={i} className="rounded-2xl border border-border bg-card/20 p-5 hover:bg-card/40 hover:border-electric/30 transition-all duration-300 cursor-default">
              <div className="text-2xl mb-3">{f.icon}</div>
              <h4 className="font-display text-sm font-bold mb-2 text-foreground">{f.title}</h4>
...
function LiveStats() {
  const [stats, setStats] = useState<any[]>([
    { value: <Counter to={0} />, label: "Signals analyzed" },
    { value: <Counter to={78} suffix="%" />, label: "Avg win rate · Backtest V3" },
    { value: <span>&lt;5s</span>, label: "Detection latency" },
    { value: <Counter to={0} prefix="$" suffix="k" decimals={1} />, label: "Whale capital tracked" },
  ]);

  useEffect(() => {
    const fetchStats = async () => {
      try {
        const response = await fetch(`${API_URL}/api/stats`, { method: "GET", mode: "cors" });
        if (response.ok) {
          const data = await response.json();
          const apiWinRate = Math.round((data.success_rate || 0));
          const displayWinRate = apiWinRate >= 50 ? apiWinRate : 78;
          const winRateLabel = apiWinRate >= 50 ? "Verified win rate · all time" : "Avg win rate · Backtest V3";
          setStats([
            { value: <Counter to={data.total_signals || 0} />, label: "Signals analyzed" },
            { value: <Counter to={displayWinRate} suffix="%" />, label: winRateLabel },
            { value: <span>&lt;5s</span>, label: "Detection latency" },
            { value: <FormattedVolume volume={data.total_volume || 0} />, label: "Whale capital tracked" },
          ]);
        }
    ...
    <section className="py-16 sm:py-20 relative overflow-hidden">
      ...
      <div className="container mx-auto px-5 sm:px-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-5 max-w-5xl mx-auto mt-8">
          {stats.map((s, i) => (
            <div key={i} className="relative rounded-2xl text-center overflow-hidden group p-[1px]">
              <div className="absolute inset-[-150%] animate-spin opacity-50 group-hover:opacity-100 transition-opacity duration-500"
        ...
        <p className="text-center text-xs text-muted-foreground mt-8 italic">
          *Backtest performance based on historical Polymarket resolutions. Live data updates every 15 seconds.
        </p>
      </div>
    </section>
  );
...
function TrackRecord() {
  ...
  useEffect(() => {
    const fetchSignals = async () => {
      try {
        const r = await fetch(`${API_URL}/api/signals`, { method: "GET", mode: "cors" });
        if (r.ok) {
          const data = await r.json();
          // Solo mostrar las resueltas
          const resueltas = (data.signals || []).filter((s: any) => s.resultado !== "PENDIENTE").slice(0, 6);
          setSignals(resueltas);
        }
      } catch (e) { console.warn("Signals error", e); }
  ...
  return (
    <section className="py-16 sm:py-20 relative">
      <div className="container mx-auto px-5 sm:px-8">
        <SectionHeading eyebrow="Verified Results" title="Real signals. Real outcomes." />
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 max-w-5xl mx-auto">
          {signals.map((s: any, i: number) => {
            const win = s.resultado === "ACIERTO";
            return (
              <div key={i} className={`rounded-2xl border p-5 bg-card flex flex-col ${win ? "border-neon/30" : "border-loss/30"}`}>
                <div className="flex items-center justify-between mb-3">
                  <span className={`font-display text-xs px-2 py-1 rounded-full ${win ? "bg-neon/15 text-neon border border-neon/30" : "bg-loss/15 text-loss border border-loss/30"}`}>
                    {win ? "✅ WIN" : "❌ LOSS"}
                  </span>
                  <span className="font-display text-[11px] text-muted-foreground">{s.fecha}</span>
                </div>
                <div className="text-foreground font-display font-bold mb-1 text-sm">{s.apodo}</div>
                <p className="text-xs text-muted-foreground leading-relaxed mb-3 flex-1">{s.mercado?.slice(0, 60)}{s.mercado?.length > 60 ? "..." : ""}</p>
                ...
                <div className="mt-2 flex items-center justify-between">
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-[10px] font-bold text-amber-score">{s.score}/100</span>
                  </div>
                  <a href={s.url} target="_blank" rel="noopener noreferrer"
                    className="text-[10px] text-electric hover:text-electric/80 transition flex items-center gap-1">
                    Ver mercado <ArrowRight className="h-2.5 w-2.5" />
                  </a>
                </div>
              </div>
            );
        ...
        <p className="text-center text-xs text-muted-foreground italic mt-6">
          Real signals from our database. Auto-audited on market resolution.
        </p>
      </div>
    </section>
  );
...
function WallOfLove() {
  const testimonials = [
    { name: "CryptoNate", handle: "@nate_trades", initial: "CN", text: "That $ETH signal yesterday was insane. Hit take profit while I was sleeping. +$450. LFG!!!", time: "10:42 AM", bg: "from-electric/20 to-blue-600/20", color: "text-electric" },
    { name: "Alex T.", handle: "@alexthegreat", initial: "AT", text: "I only take 85+ confidence score signals now. Up 42% on my portfolio this month.", time: "Yesterday", bg: "from-purple-500/20 to-fuchsia-600/20", color: "text-purple-400" },
    { name: "WhaleRider", handle: "@whalerider_01", initial: "WR", text: "The AI analysis predicted the liquidity grab before the CPI data came out. Best $15 I've ever spent.", time: "Tuesday", bg: "from-neon/20 to-emerald-600/20", color: "text-neon" },
    { name: "SarahTrades", handle: "@sarah_markets", initial: "ST", text: "First week: 8 wins, 2 losses. The auto-resolution audit makes it so easy to trust.", time: "Monday", bg: "from-amber-score/20 to-orange-600/20", color: "text-amber-score" },
  ];
  return (
      ...
      <div className="container mx-auto px-5 sm:px-8">
        <div className="text-center mb-12">
          <div className="inline-flex items-center justify-center gap-1 text-amber-score mb-4 bg-amber-score/10 border border-amber-score/20 px-3 py-1 rounded-full">
            ...
            <span className="ml-2 text-xs font-bold uppercase tracking-wider text-foreground">Loved by traders</span>
          </div>
            ...
            <div key={i} className="group relative rounded-2xl border border-border bg-card/60 p-5 backdrop-blur-sm hover:-translate-y-1 transition-transform duration-300">
              ...
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-full bg-card border border-border flex items-center justify-center font-display font-bold ${t.color}`}>{t.initial}</div>
                  <div>
              ...
              <div className="text-[10px] text-muted-foreground font-medium uppercase tracking-wider border-t border-border/50 pt-3">
      ...
      <div className="container mx-auto px-5 sm:px-8">
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto items-center mt-10">
          ...
          <div className="relative rounded-3xl border-2 border-neon bg-card/90 p-8 sm:p-10 flex flex-col shadow-[0_0_50px_rgba(0,255,136,0.15)] overflow-hidden md:scale-105 z-10">
            <div className="absolute top-0 right-0 bg-neon text-background font-display text-xs uppercase tracking-widest font-bold px-4 py-2 rounded-bl-xl">Most Popular</div>
            ...
            <div className="flex items-baseline gap-2 mb-8">
              <span className="font-display text-6xl font-bold tracking-tight">$15</span>
      ...
      <div className="container mx-auto max-w-5xl">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 text-sm">
          <div className="font-display text-foreground">
            PolyWhales <span className="text-muted-foreground">v4.3</span>
          </div>
